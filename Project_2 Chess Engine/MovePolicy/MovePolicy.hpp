...
class MovePolicy : public Board{
    ...
    unsigned long long diagonal_right_up_move_mask = a | one;
    unsigned long long diagonal_right_down_move_mask = a | eight;

    unsigned long long diagonal_left_up_move_mask = h | one; 
    unsigned long long diagonal_left_down_move_mask = h | eight;

    unsigned long long knight_up_up_left_move_mask = h | g | one;
    unsigned long long knight_up_left_left_move_mask = h | one | two; 

    unsigned long long knight_down_left_left_move_mask = h | g | eight; 
    unsigned long long knight_down_down_left_move_mask = h | seven | eight;

    unsigned long long knight_up_up_right_move_mask = a | one | two; 
    unsigned long long knight_up_right_right_move_mask = b | a | one; 

    unsigned long long knight_down_right_right_move_mask = b | a | eight;
    unsigned long long knight_down_down_right_move_mask = a | seven | eight;
